...
const Headline = ({
  ...
  const handleHeadingClick = () => {
    ...
    setIsActive(true); // Set isActive to true after clicking
  };

  const attributes = {
    ...propAttributes,
    id: propAttributes?.slug || slug,
    className: classnames('group cursor-pointer hover:underline', { 'active': isActive }, propAttributes?.className),
  ...
  return (
    <Tag attributes={attributes}>
      ...
      {isActive && <span className='text-slate-300 inline-block ml-2'>¶</span>}
    </Tag>
  );
};
...
const Headline1Tag = ({ children, attributes }: TagProps) => (
  <h1 {...attributes} className={classnames(attributes?.className, 'text-h1mobile md:text-h1  font-bold pt-10 mb-6')}>
    {children}
  </h1>
);
const Headline2Tag = ({ children, attributes }: TagProps) => (
  <h2 {...attributes} className={classnames(attributes?.className, 'text-h2mobile md:text-h2 font-semibold mt-10 mb-4')}>
    {children}
  </h2>
);
const Headline3Tag = ({ children, attributes }: TagProps) => (
  <h3 {...attributes} className={classnames(attributes?.className, 'text-h3mobile md:text-h3 font-semibold mt-6 mb-3')}>
    {children}
  </h3>
);
const Headline4Tag = ({ children, attributes }: TagProps) => (
  <h4 {...attributes} className={classnames(attributes?.className, 'text-h4mobile md:text-h4 font-semibold mt-4 mb-2')}>
    {children}
  </h4>
);
