...
describe('NavigationButtons Component', () => {
  ...
  it('should render both navigation buttons correctly', () => {
    ...
    cy.get('[data-test="nav-button-prev"]')
      .within(() => {
        cy.get('[data-test="nav-button-text"]').should('have.text', 'Go Back');
        cy.get('[data-test="nav-button-label"]').should('have.text', defaultProps.prevLabel);
        cy.get('[data-test="nav-button-icon"]').should('exist');
      });
    
    // Check next button content
    cy.get('[data-test="nav-button-next"]')
      .within(() => {
        cy.get('[data-test="nav-button-text"]').should('have.text', 'Up Next');
        cy.get('[data-test="nav-button-label"]').should('have.text', defaultProps.nextLabel);
        cy.get('[data-test="nav-button-icon"]').should('exist');
      });
  });
  ...
  it('should render only previous button when next props are missing', () => {
    ...
    cy.get('[data-test="nav-button-prev"]')
      .within(() => {
        cy.get('[data-test="nav-button-text"]').should('have.text', 'Go Back');
        cy.get('[data-test="nav-button-label"]').should('have.text', propsWithOnlyPrev.prevLabel);
      });
    
    // Next button should be a placeholder div
    cy.get('[data-test="nav-button-next"]').should('not.exist');
    cy.get('[class*="w-1/2"]').should('have.length', 2);
  });
  ...
  it('should render only next button when prev props are missing', () => {
    ...
    cy.get('[data-test="nav-button-next"]')
      .within(() => {
        cy.get('[data-test="nav-button-text"]').should('have.text', 'Up Next');
        cy.get('[data-test="nav-button-label"]').should('have.text', propsWithOnlyNext.nextLabel);
      });
  ...
  it('should apply correct styling classes', () => {
    ...
    // Check card styling
    cy.get('.h-full.cursor-pointer.border-gray-200.p-4.text-center.shadow-md').should('exist');
    
    // Check button styling
    cy.get('.w-full.gap-5.p-0.text-\\[18px\\].hover\\:bg-transparent').should('exist');
  ...
  it('should handle hover effects correctly', () => {
    ...
    cy.get('.dark\\:shadow-xl.dark\\:hover\\:shadow-lg.dark\\:drop-shadow-lg').should('exist');
  });
  ...
  it('should handle mixed valid and invalid props', () => {
    ...
    cy.get('[data-test="nav-button-prev"]')
      .within(() => {
        cy.get('[data-test="nav-button-label"]').should('have.text', mixedProps.prevLabel);
      });
